...
class Util {
  //生成随机字符串
  generateRandomStr(type = "string", length = 5) {
  ...
  //验证短信验证码
  async validateSmsCode({ mobile, code, type }) {
    const cacheCode = await redis.get(`${mobile}:${type}`);
    return cacheCode === code;
  }
  async clearSmsCode({ mobile, type }) {
    await redis.del(`${mobile}:${type}`);
  }
  encryptThreeDESECB(plaintext) {
  ...
  getRedisLock(lockName, ttl = 1000) {
  ...
  saveFaceImage(uniqId, basePath, faceImage) {
